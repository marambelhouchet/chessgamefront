...
function renderDiceFace(v) {
  return (PIP[v] || []).map(([x, y]) => `<circle cx="${x}" cy="${y}" r="5" fill="#e8588a"/>`).join('');
}
...
function animateDice(finalVal, elId) {
  return new Promise(resolve => {
    ...
    const iv = setInterval(() => {
      ...
      if (el) el.innerHTML = renderDiceFace(v);
      if (++count >= 9) {
        clearInterval(iv);
        if (el) el.innerHTML = renderDiceFace(finalVal);
        resolve();
      }
    }, 65);
  });
...
/* ── Startup rolls (who goes first) ── */
async function startRoll(player) {
  const val = Math.ceil(Math.random() * 6);
  startRolls[player - 1] = val;
  playDiceSound();
  ...
  if (player === 1) {
    ...
    document.getElementById('roll-p2').textContent = val;
    ...
    const winner  = a > b ? CFG.p1 : CFG.p2;
    setTimeout(() => {
      ...
      p.textContent = `⚔ ${winner} commence !`;
      const mc = document.getElementById('modal-content');
      const bs = document.getElementById('btn-start');
      mc.insertBefore(p, bs);
